...
def insert_data(conn, date, name, rate):
    ...
    cursor = conn.cursor()
    sql = '''
    INSERT OR IGNORE INTO Date_compare (date, name, rate) VALUES (?, ?, ?)
    '''
    cursor.execute(sql, (date, name, rate))
    conn.commit()
...
def analyze_sector_impacts(filename):
    ...
    # 连接数据库
    conn = connect_db()
    today = date.today().strftime("%Y-%m-%d")  # 获取今天的日期

    # 遍历正值结果并插入数据库
    for sector, count in adjusted_positive_counts.items():
        insert_data(conn, today, sector, count)
