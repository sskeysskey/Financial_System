...
(function () {
    ...
    function getColumnIndices(table) {
        ...
        return {
            ...
            open: headers.findIndex(h => /0pen|Open/i.test(h)), // 兼容 HTML 中可能出现的 0pen
    ...
    function main() {
        ...
        rows.forEach(r => {
            ...
            const maxIndex = Math.max(cols.date, cols.close, cols.volume, cols.open, cols.high, cols.low);
            ...
            // 解析 Open, High, Low
            let open = null, high = null, low = null;
            if (cols.open >= 0 && cells[cols.open]) {
                const val = parseFloat(cells[cols.open].textContent.trim().replace(/,/g, ''));
                if (!isNaN(val)) open = val;
            }
            if (cols.high >= 0 && cells[cols.high]) {
                const val = parseFloat(cells[cols.high].textContent.trim().replace(/,/g, ''));
                if (!isNaN(val)) high = val;
            }
            if (cols.low >= 0 && cells[cols.low]) {
                const val = parseFloat(cells[cols.low].textContent.trim().replace(/,/g, ''));
                if (!isNaN(val)) low = val;
            }

            if (!isNaN(d) && !isNaN(price)) {
                scraped.push({ date, price, volume, open, high, low });
            }
        });
        ...
        let csv = 'date,price,volume,open,high,low\n';
        scraped.forEach(e => {
            const v = e.volume !== null ? e.volume : '';
            const o = e.open !== null ? e.open : '';
            const h = e.high !== null ? e.high : '';
            const l = e.low !== null ? e.low : '';
            csv += `${e.date},${e.price},${v},${o},${h},${l}\n`;
        });
