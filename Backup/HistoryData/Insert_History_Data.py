...
def process_csv_file(csv_filepath, symbol, group_name, db_path):
    ...
    try:
        with open(csv_filepath, 'r', newline='', encoding='utf-8') as f_csv:
            ...
            has_open = 'open' in header
            has_high = 'high' in header
            has_low = 'low' in header
            ...
            for lineno, row in enumerate(reader, start=2): # lineno 从2开始，因为表头是第1行
                ...
                volume_val = 0
                if has_vol_in_csv:
                    vol_str = row[header.index('volume')].strip()
                    if vol_str:
                        try: volume_val = int(vol_str)
                        except ValueError: pass

                # 提取 open, high, low
                open_val = None
                if has_open and row[header.index('open')].strip():
                    try: open_val = float(row[header.index('open')].strip())
                    except ValueError: pass
                
                high_val = None
                if has_high and row[header.index('high')].strip():
                    try: high_val = float(row[header.index('high')].strip())
                    except ValueError: pass
                
                low_val = None
                if has_low and row[header.index('low')].strip():
                    try: low_val = float(row[header.index('low')].strip())
                    except ValueError: pass

                # 将所有字段打包
                rows.append((date_val, symbol, price_val, volume_val, open_val, high_val, low_val))
    ...
    has_ohl_in_db = 'open' in cols and 'high' in cols and 'low' in cols

    final_rows_for_db = []
    upsert_sql = ""

    # 动态构建 SQL
    if has_ohl_in_db and has_vol_in_db:
        upsert_sql = f"""
        ...
        ON CONFLICT(date, name) DO UPDATE SET
            ...
            volume = excluded.volume;
        """
        final_rows_for_db = [r[:4] for r in rows]
    else:
        # 情况2：数据库表没有 'volume' 列
        ...
        ON CONFLICT(date, name) DO UPDATE SET
            price = excluded.price;
        """
        final_rows_for_db = [r[:3] for r in rows]
    # --- 执行批量写入 ---
    try:
