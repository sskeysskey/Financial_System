...
def fetch_data(driver, url, blacklist):
    ...
    try:
        ...
        def get_rows():
            # 使用JavaScript滚动到表格底部以确保所有数据加载
            driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
            time.sleep(1)  # 给加载一些时间
            return driver.find_elements(By.CSS_SELECTOR, "table tbody tr")
            
        def extract_row_data_with_retry(row, max_retries=3):
            for attempt in range(max_retries):
                try:
                    # 重新获取新鲜的row元素
                    row_id = row.get_attribute("id") or row.get_attribute("data-row-index")
                    if row_id:
                        row = driver.find_element(By.ID, row_id)
                    
                    # 使用JavaScript执行滚动
                    driver.execute_script("arguments[0].scrollIntoView(true);", row)
                    time.sleep(0.5)  # 给视图转换一些时间
                    
                    # 使用显式等待获取每个元素
                    wait = WebDriverWait(driver, 10)
                    
                    symbol = wait.until(EC.presence_of_element_located(
                        (By.CSS_SELECTOR, "a[data-testid='table-cell-ticker'] span.symbol")
                    )).text.strip()
                    ...
                    )).get_attribute("data-value").strip()
                    
                    # 使用JavaScript获取值以避免stale element
                    volume_script = """
                        return arguments[0].querySelector('td:nth-child(8)').textContent;
                    """
                    volume = driver.execute_script(volume_script, row).strip()
                    
                    market_cap_script = """
                        return arguments[0].querySelector('td:nth-child(10)').textContent;
                    """
                    market_cap = driver.execute_script(market_cap_script, row).strip()
                    
                    pe_ratio_script = """
                        return arguments[0].querySelector('td:nth-child(11)').textContent;
                    """
                    pe_ratio = driver.execute_script(pe_ratio_script, row).strip()
                    ...
                    if attempt == max_retries - 1:
                        raise
                    print(f"Stale element, retrying... (attempt {attempt + 1})")
                    time.sleep(1)  # 在重试之前等待
                    continue
        
        rows = get_rows()
        for row in rows:
            try:
                data = extract_row_data_with_retry(row)
                if data:
                    symbol, name, price, volume, market_cap, pe_ratio = data
