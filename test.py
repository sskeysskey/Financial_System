...
CONFIG = {
    ...
    # ========== 新增：条件6 (超跌反弹策略) 参数 ==========
    # A: 最新财报收盘价 vs 上次财报收盘价 的跌幅
    # B: 最新收盘价 vs 最新财报收盘价 的跌幅
    "COND6_DROP_A_THRESHOLD": 0.25,     # 财报间跌幅阈值 A (25%)
    "COND6_DROP_B_HIGH": 0.15,          # 如果 A <= 25%，则 B 需要 > 15%
    "COND6_DROP_B_LOW": 0.09,           # 如果 A > 25%，则 B 只要 > 9%
}
...
def check_new_condition_6(data, config, log_detail, symbol_to_trace):
    ...
    # 2. 数据完整性检查
    # 需要至少2次财报价格才能计算 A (Previous ER vs Latest ER)
    if len(all_er_prices) < 2 or latest_price is None:
        if is_tracing: log_detail(f"  - 结果: False (数据不足: 财报记录数={len(all_er_prices)}, 最新价={latest_price})")
        return False

    p_er_latest = all_er_prices[-1]   # 最新一次财报收盘价
    p_er_prev = all_er_prices[-2]     # 上一次财报收盘价

    if p_er_prev <= 0 or p_er_latest <= 0:
        if is_tracing: log_detail(f"  - 结果: False (价格无效: Prev={p_er_prev}, LatestER={p_er_latest})")
        return False

    # 3. 计算跌幅
    # 跌幅 A: 最新一次财报 比 前一次财报 低 A% -> (Prev - Latest) / Prev
    drop_a = (p_er_prev - p_er_latest) / p_er_prev
    
    # 跌幅 B: 最新收盘价 比 最近一次财报收盘价 低 B% -> (LatestER - Current) / LatestER
    drop_b = (p_er_latest - latest_price) / p_er_latest

    # 4. 获取阈值
    threshold_a = config.get("COND6_DROP_A_THRESHOLD", 0.25)
    threshold_b_high = config.get("COND6_DROP_B_HIGH", 0.15)
    threshold_b_low = config.get("COND6_DROP_B_LOW", 0.09)
    ...
    # 5. 逻辑判断
    # 如果 A > 25%，则 B 超过 9% 即可
    if drop_a > threshold_a:
        required_b = threshold_b_low
        passed = drop_b > threshold_b_low
        if is_tracing: log_detail(f"  - 路径: 跌幅A ({drop_a:.2%}) > 25%，使用低门槛 B > {threshold_b_low:.2%}")
    # 如果 A <= 25%，则 B 需要超过 15%
    else:
        required_b = threshold_b_high
        passed = drop_b > threshold_b_high
        if is_tracing: log_detail(f"  - 路径: 跌幅A ({drop_a:.2%}) <= 25%，使用高门槛 B > {threshold_b_high:.2%}")

    if is_tracing:
        log_detail(f"  - 上次财报价: {p_er_prev:.2f}")
        log_detail(f"  - 最新财报价: {p_er_latest:.2f} (跌幅A: {drop_a:.2%})")
        log_detail(f"  - 最新收盘价: {latest_price:.2f} (跌幅B: {drop_b:.2%})")
        log_detail(f"  - 判断: 跌幅B {drop_b:.2%} > 要求 {required_b:.2%} -> {passed}")
        log_detail(f"  - 结果: {passed}")
...
def check_entry_conditions(data, symbol_to_trace, log_detail):
    """
    此函数检查入口条件 (条件1 OR 条件2 OR 条件3 OR 条件4 OR 条件5 OR 条件6)。
    返回:
        (bool, bool, bool): 一个元组 (passed_any_condition, passed_condition_5, passed_condition_6)
    """
    symbol = data.get('symbol')
    is_tracing = (symbol == symbol_to_trace)
    if is_tracing: 
    ...
    # --- 数据预检 ---
    er_pcts = data.get('all_er_pcts', [])
    # 注意：条件6只需要价格，不需要涨跌幅pct，但为了统一预检，这里保持原样，
    # 或者如果条件6允许没有pct数据，可以放宽这里。鉴于大部分逻辑依赖pct，暂时保留。
    ...
    passed_new_cond6 = check_new_condition_6(data, CONFIG, log_detail, symbol_to_trace) # 新增
    ...
    if is_tracing:
        if passed_any:
            ...
            if passed_new_cond6: reasons.append("条件6")
            log_detail(f"\n--- [{symbol}] 入口条件通过 (原因: {'、'.join(reasons)})。")
        else:
            log_detail(f"\n--- [{symbol}] 入口条件失败。六个入口条件均未满足。")

    # 返回元组增加 passed_new_cond6
...
def run_processing_logic(log_detail):
    ...
    def perform_filter_pass(symbols_to_check, drop_large, drop_small, pass_name):
        ...
        for symbol in symbols_to_check:
            ...
            # 步骤B: 应用通用过滤器。
            # 关键逻辑：如果通过了条件5 或 条件6，则将 skip_drawdown 设为 True
            # 条件6是寻找连续下跌，自然不适用常规的“高点回撤”过滤，但仍需遵守10日最低价和成交额限制
    ...
    if len(pass1_valid) < min_size_pe_valid:
        # 注意：这里重新跑严格组，条件6如果被严格组包含，同样会生效并跳过drawdown检查
