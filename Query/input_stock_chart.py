...
def input_mapping(root, sector_data, compare_data, marketcap_pe_data):
    # 获取用户输入
    prompt = "请输入"
    ...
    if user_input is None:
        ...
        #             compare = compare_data.get(input_trimmed, "N/A")
        #             marketcap_pe = marketcap_pe_data.get(input_trimmed, (None, 'N/A'))
        #             marketcap, pe = marketcap_pe
        #             plot_financial_data(db_path, sector, input_trimmed, compare, marketcap, pe)
        ...
        for sector, categories in sector_data.items():
            for category, names in categories.items():
                if upper_input in names:
                    ...
                    marketcap_pe = marketcap_pe_data.get(upper_input, (None, 'N/A'))
                    marketcap, pe = marketcap_pe
                    plot_financial_data(db_path, sector, upper_input, compare, marketcap, pe)
                ...
                for category, names in categories.items():
                    for name in names:
                        if re.search(lower_input, name.lower()):
                            ...
                            marketcap, pe = marketcap_pe
                            plot_financial_data(db_path, sector, name, compare, marketcap, pe)
                            found = True
                            close_app(root)
                            break
                    if found:
...
if __name__ == '__main__':
    ...
    input_mapping(root, sector_data, compare_data, marketcap_pe_data)
