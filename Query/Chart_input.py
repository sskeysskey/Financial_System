...
def plot_financial_data(db_path, table_name, name, compare, share, marketcap, pe, json_data,
    ...
    def draw_subtitle():
        ...
        # 解析 Compare 数值用于可能的颜色判断（虽然 Check_Options 中 Badge 主要是蓝底白字）
        try:
            # 提取百分比数值，例如 "NVDA: -2.5%" -> -2.5
            clean_s = re.sub(r'[^\d\.\-]', '', compare_str.split(':')[-1])
            compare_val = float(clean_s)
        except:
            compare_val = 0.0

        # 3. 定义样式常量 (复刻 Check_Options 的 Hex 颜色)
        COLOR_PRI_UP = "#FF4500"  # 鲜红 (Primary)
        COLOR_PRI_DN = "#00FA9A"  # 鲜绿 (Primary)
        COLOR_SEC_UP = "#E57373"  # 淡红 (Secondary)
        COLOR_SEC_DN = "#81C784"  # 淡绿 (Secondary)
        COLOR_NULL   = "#DDDDDD"  # 灰色
        ...
        def get_style(val, role):
            ...
            is_up = val > 0
            if role == 'primary':
                color = COLOR_PRI_UP if is_up else COLOR_PRI_DN
                weight = 'bold'
                size = 18 # 对应 Check_Options 的 "20px"
            elif role == 'secondary':
                color = COLOR_SEC_UP if is_up else COLOR_SEC_DN
                weight = 'normal'
                size = 12 # 对应 Check_Options 的 "14px"
            else:
                color = COLOR_NULL
                weight = 'normal'
                size = 13
