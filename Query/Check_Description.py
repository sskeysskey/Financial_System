...
def show_description(name, descriptions):
    root = tk.Tk()
    root.withdraw()  # 隐藏主窗口
...
def find_in_json(name, data):
    """在JSON数据中查找名称为name的股票或ETF"""
    for item in data:
        if item['name'] == name:
            return item
    return None
...
Copy_Command_C()
name = pyperclip.paste().replace('"', '').replace("'", "")

# 先在stocks中查找
result = find_in_json(name, json_data.get('stocks', []))

# 如果在stocks中没有找到，再在etfs中查找
if not result:
    result = find_in_json(name, json_data.get('etfs', []))

# 如果找到结果，显示信息
if result:
    show_description(name, result)
else:
    messagebox.showinfo("失败", f"未找到名称为 {name} 的股票或ETF！")