...
class EarningsSearchApp(QWidget):
    # 修改构造函数以接受一个可选参数
    def __init__(self, initial_symbol=None):
        super().__init__()
        ...
        # --- 新增逻辑 ---
        # 检查是否有初始 symbol 传入，并且文件已成功加载
        if hasattr(self, 'file_not_found') and self.file_not_found:
            return

        if initial_symbol:
            # 如果有，自动填充并搜索
            self.search_input.setText(initial_symbol)
            self.perform_search()
...
def main():
    ...
    initial_symbol = None
    if len(sys.argv) > 1:
        initial_symbol = sys.argv[1] # 第一个参数就是我们要的symbol

    # 将参数传递给主窗口
    ex = EarningsSearchApp(initial_symbol=initial_symbol)
