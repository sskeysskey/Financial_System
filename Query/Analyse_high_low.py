...
def analyze_stock_data(file_path):
    ...
    newhigh_counts = data[data['Status'].str.contains('newhigh')].groupby('Industry').size()
    newlow_counts = data[data['Status'].str.contains('newlow')].groupby('Industry').size()
    ...
    output_filename = f"A_{today}.txt"
    with open(output_filename, 'w') as f:
        f.write("NewHighs:\n")
        for industry in newhigh_sorted.index:
            f.write(f"{industry:25s} {newhigh_sorted[industry]:>7.4f} {newhigh_percentage[industry]:>6.2f}%\n")
        f.write("\nNewLows:\n")
        for industry in newlow_sorted.index:
            f.write(f"{industry:25s} {newlow_sorted[industry]:>7.4f} {newlow_percentage[industry]:>6.2f}%\n")
...
def main():
    file_path = '/Users/yanzhang/Documents/News/Analyse_Stock.txt'
