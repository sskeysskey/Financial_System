...
blacklist = ["CTA-PA", "FWONK", "FOXA", "NWSA", "PARAA", "LSXMA",
    ...
    "BAC-PE", "BAC-PK", "BAC-PB", "BML-PL", "BML-PJ", "WFC-PY",
...
def fetch_data(driver, url):
    ...
    for quote_link in quote_links:
        ...
        if is_blacklisted(symbol):
            print(f"Symbol {symbol} is blacklisted and will be skipped.")
            continue  # 跳过黑名单中的符号
        symbol_xpath = f'//fin-streamer[@data-symbol="{symbol}"]'
        pe_ratio_xpath = f'//td[@aria-label="PE Ratio (TTM)"]'
        try:
            ...
            pe_ratio = pe_ratio_element.text  # 使用 .text 获取元素文本内容
            # 现在我们返回所有符号的市值，不论它们的大小
...
def update_json(data, sector, file_path, output, log_enabled):
    with open(file_path, 'r+') as file:
        ...
        for category in json_data[sector]:
            for symbol_info in json_data[sector][category]:
        ...
        for symbol, market_cap, pe_ratio in data:
            ...
            if market_cap >= 200000000000:
                ...
                continue  # 如果市值小于 5B，则不进行处理
            
            symbol_data = {'symbol': symbol, 'marketcap': f"{market_cap:.1f}", 'pe_ratio': pe_ratio}

            # 如果当前分类存在且与新分类不符，从当前分类中移除
            if current_category and current_category != category:
                json_data[sector][current_category] = [info for info in json_data[sector][current_category] if info['symbol'] != symbol]
                if log_enabled:
            ...
            # 添加到新的分类中
            json_data[sector].setdefault(category, []).append(symbol_data)
            if log_enabled:
                message = f"Added/Updated '{symbol}' with marketcap {market_cap:.1f} and pe_ratio {pe_ratio} to {category} in {sector}."
                print(message)
                output.append(message)
